...
export default function GraficaConveniosGenerales() {
  const [data, setData] = useState<{ mes: string; total: number }[]>([]);

  useEffect(() => {
    async function fetchData() {
      const res = await fetch("/api/Convenios/Estadistica/General");
  ...
  return (
    <div className="bg-white rounded-xl shadow-md w-full p-6 flex flex-col space-y-6">
      ...
      <div className="flex items-center justify-between w-full">
        <h2 className="text-2xl font-bold text-gray-800 border-l-4 border-green-400 pl-4">
          Solicitudes de Convenios Generales
        </h2>
      </div>
          ...
          <AreaChart data={data} margin={{ left: 0, right: 10 }}>
            ...
            <defs>
              <linearGradient id="generalGradient" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#53b431" stopOpacity={0.4} />
                <stop offset="100%" stopColor="#53b431" stopOpacity={0.05} />
              </linearGradient>
            </defs>
            <Area
              type="monotone"
