...
export default function LayoutAdminConvenioEspecifico({
  children,
}: {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-[#011848] text-white py-4 px-8 flex items-center justify-between">
        <h1 className="text-lg font-semibold">
          Revisión de Convenio Especificos # {id_solicitud}
        </h1>
        <Link href="/Admin/Convenios/Especificos" className="text-sm underline">
          ← Volver
        </Link>
      </header>
      ...
      <nav className="flex justify-center gap-4 border-b bg-white shadow-sm">
        {pasos.map((p) => {
          ...
          return (
            <Link
              key={p.id}
              href={`/Admin/Convenios/Especificos/${id_solicitud}/${p.id}`}
