...
export default function DetalleOfertaPage({
  ...
  if (!oferta) {
    return (
      <p className="p-10 text-center text-red-500">Vacante no encontrada</p>
    );
  }
  return (
      ...
      <div className="bg-white p-6 rounded-xl shadow flex gap-6">
        ...
        <div className="flex-1 space-y-2">
          ...
          <p className="text-gray-600">
            Publicada el:{" "}
            {getFechaLocalSinHora(oferta.fecha_publicacion).toLocaleDateString(
              "es-MX"
            )}
          </p>
          <p className="text-gray-600">
            Creada en sistema:{" "}
            {getFechaLocalSinHora(oferta.creado_en).toLocaleDateString("es-MX")}
          </p>
          <p className="mt-2 text-gray-800 leading-relaxed">
            {oferta.descripcion_general}
          </p>
        </div>
      </div>
