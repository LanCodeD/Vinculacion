      ...
      <div className='flex items-center gap-4 mb-6'>
        ...
        <div>
          ...
          <UploadFile
            ...
            onUploaded={(url) => setUser(prev => prev ? { ...prev, imagen_perfil: url } : prev)}
          />
        </div>
      </div>
        ...
        <div>
          ...
          {user.empresas.map(emp => (
            <div key={emp.id_empresas} className='mb-4 p-4 border rounded'>
              <p><strong>Nombre Comercial:</strong> {emp.nombre_comercial}</p>
              {emp.razon_social && <p><strong>Razón Social:</strong> {emp.razon_social}</p>}
              <p><strong>RFC:</strong> {emp.rfc}</p>
              {emp.direccion && <p><strong>Dirección:</strong> {emp.direccion}</p>}
              {emp.correo && <p><strong>Correo:</strong> {emp.correo}</p>}
        ...
        <div>
          ...
          {user.egresados.map(eg => (
            <div key={eg.id_egresados} className='mb-4 p-4 border rounded'>
              {eg.titulo && <p><strong>Título:</strong> {eg.titulo}</p>}
              {eg.puesto && <p><strong>Puesto:</strong> {eg.puesto}</p>}
