...
const pasos = [
  ...
  // { id: "Responsabilidades", nombre: "Responsabilidades" }, ❌ eliminado
  { id: "Eventos", nombre: "Eventos" },
  { id: "EstadoSolicitud", nombre: "Estado de la Solicitud" },
];


export default function LayoutConvenioGeneral({
  children,
}: {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-[#011848] text-white py-4 px-8 flex items-center justify-between">
        <h1 className="text-lg font-semibold">
          Convenio General #{id_solicitud}
        </h1>
        <Link
          href="/Convenios/Generales"
      ...
      <nav className="flex justify-center gap-4 border-b bg-white shadow-sm">
        {pasos.map((p) => {
          ...
          return (
            <Link
              key={p.id}
              href={`/Convenios/Generales/${id_solicitud}/${p.id}`}
