...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const vacante = await prisma.ofertas.findUnique({ where: { id_ofertas: id } });
    if (!vacante || vacante.empresas_id !== empresa.id_empresas)
      return NextResponse.json({ ok: false, error: "Acceso denegado" }, { status: 403 });

    await prisma.ofertas.delete({ where: { id_ofertas: id } });
  ...
  try {
    ...
    const updated = await prisma.ofertas.update({
      ...
      data: {
        ...
        oferta_estados_id: 2, // vuelve a pendiente de revisión tras editar
      },
    });
