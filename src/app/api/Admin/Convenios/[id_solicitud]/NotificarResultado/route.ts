// /app/api/Admin/Convenios/[id_solicitud]/NotificarResultado/route.ts
import { NextRequest, NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";
import { getSessionUser } from "@/lib/auth";
import { enviarCorreo } from "@/lib/mailer";
import { plantillaResultadoRevisionEspecifico } from "@/lib/PlantillasCorreos/resultadoRevisionEspecifico";
...
export async function POST(
  ...
  try {
    ...
    const tipo = fueRechazada
      ? "solicitud_correccion_especifico"
      : fueAprobada
      ? "solicitud_aprobada_especifico"
      : "solicitud_revision_especifico";

    const titulo = fueRechazada
      ? "Tu solicitud de Convenio Específico requiere correcciones"
      : fueAprobada
      ? "Tu solicitud de Convenio Específico ha sido aprobada"
      : "Tu solicitud de Convenio Específico sigue en revisión";
    ...
    await enviarCorreo({
      ...
      subject: titulo,
      html: plantillaResultadoRevisionEspecifico({
