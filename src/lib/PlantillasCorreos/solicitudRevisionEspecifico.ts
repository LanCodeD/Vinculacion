// /lib/PlantillasCorreos/solicitudRevisionEspecifico.ts
import { plantillaCorreoBase } from "./plantillaCorreoBase";

export function plantillaSolicitudRevisionEspecifico({
  adminNombre,
  usuarioNombre,
  idSolicitud,
  fechaEnvio,
  ...
  return plantillaCorreoBase({
    titulo: "Nueva solicitud de Convenio Específico enviada para revisión",
    mensaje: `
      <div style="font-family:Arial, sans-serif; color:#333; line-height:1.6;">
        <p style="font-size:15px;">Hola <strong>${adminNombre}</strong>,</p>
        
        <p style="font-size:14px; margin-bottom:10px;">
          El usuario <strong>${usuarioNombre}</strong> ha enviado la solicitud de un Convenio Específico
    ...
    `,
    botonTexto: "Revisar solicitud específica",
    botonUrl,
  });
}
